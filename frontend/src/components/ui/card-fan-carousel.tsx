...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full pt-0 pb-2 lg:pb-4 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
          <div ref={containerRef} className="fan-layout relative w-full max-w-[80rem] h-[20rem] sm:h-[24rem] md:h-[26rem] lg:h-[28rem]">
          {cards.map((card, index) => {
            const cardClasses =
              "fan-card absolute inset-0 m-auto w-28 h-44 sm:w-36 sm:h-56 md:w-40 md:h-64 lg:w-44 lg:h-72 rounded-2xl overflow-hidden shadow-xl cursor-pointer";
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl">
                <img
                  src={card.imgUrl}
                  loading="lazy"
                  alt={card.alt || `Destination ${index + 1}`}
                  className="absolute inset-0 w-full h-full object-cover z-10"
                />
                {/* subtle label overlay */}
                {card.alt && (
                  <div className="absolute bottom-0 inset-x-0 z-20 bg-gradient-to-t from-black/70 to-transparent px-2 py-2">
                    <p className="text-white text-[10px] sm:text-xs font-bold truncate">{card.alt}</p>
                  </div>
                )}
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={cardClasses}>
                {image}
              </a>
            ) : (
